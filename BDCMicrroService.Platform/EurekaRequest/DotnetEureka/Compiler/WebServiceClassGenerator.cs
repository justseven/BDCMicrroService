...
namespace DotnetEureka.Compiler
{
    public static class WebServiceClassGenerator
    {
        ...
        static WebServiceClassGenerator()
        {
            ...
            var DotnetFeign = DiscoverySupport.Configuration.GetSection("DotnetFeign");
            var debug = DotnetFeign.GetSection("debug");
        ...
        public static string RootNamespace { get; set; } = "DotnetEureka.Wrapper";

        // ReSharper disable once MemberCanBePrivate.Global
        public static bool GenerateSealedClasses { get; set; } = true;

        //internal static string GenerateInterfaceSource(string srvName, string route,string requestMethod, string returnType,List<ParameterModel> paramsList, out string fullClassName)
        //{
        //    var className = $"interface_{Guid.NewGuid().ToString("N")}";
        ...
        //        var baseRoute = route;
        //        var sealedString = "public";


        //        Logger.LogTrace("Beginning to generate source");

        //        var classBuilder = new StringBuilder();
        //        classBuilder.AppendLine($"using DotnetEureka;");
        //        classBuilder.AppendLine($"namespace {RootNamespace}");
        //        classBuilder.AppendLine("{");
        //        classBuilder.AppendLine("    // Generated by DotnetEureka");
        //        classBuilder.AppendLine($"[DotnetEurekaClient(\"{serviceName}\")]");
        //        classBuilder.AppendLine($"[Route(\"{baseRoute}\")]");
        //        classBuilder.AppendLine(
        //            $"    public interface {className} ");
        //        classBuilder.AppendLine("    {");
        //        classBuilder.Append($"[{requestMethod}]");//
        //        classBuilder.Append($"{returnType} GetInfo(");

        //        foreach (var p in paramsList)
        //        {
        //            classBuilder.Append($"[{p.ParamAttribute}(\"{ p.ParamName}\")]{p.ParamType} {p.ParamName});");
        //        }
        //        classBuilder.AppendLine("    }");
        //        classBuilder.AppendLine("}");
        ...
        public static string GenerateSource(TypeInfo typeInfo, out string fullClassName)
        {
            ...
            using (Logger.BeginScope("Generator {0}:", className))
            {
                var serviceName = typeInfo.GetCustomAttribute<DotnetEurekaClientAttribute>().Name;
                ...
                classBuilder.AppendLine("    // Generated by DotnetEureka");
                classBuilder.AppendLine(
                    $"    public {sealedString} class {className} : DotnetEureka.HttpSupport.DiscoveryAwareBase, {typeInfo.FullName}");
                classBuilder.AppendLine("    {");
        ...
        private static void BuildMethod(StringBuilder classBuilder, MethodInfo method)
        {
            ...
            classBuilder.AppendLine("            var reqParams = new System.Collections.Generic.Dictionary<string,string>();");
            ...
            classBuilder.AppendLine($"            var request = CreateRequest({ToCompilableName(attribute.Method)}, \"{attribute.Path}\", variables, reqParams);");
            ...
            {
                classBuilder.AppendLine($"            request.Headers.Add(\"{entry.Key}\", {entry.Value.Name}{(entry.Value.ParameterType.IsClass ? "?" : "")}.ToString());");
            }
            ...
            if (hasContent)
            {
                ...
                {
                    classBuilder.AppendLine($"            content.Headers.Add(\"{entry.Key}\", {entry.Value.Name}{(entry.Value.ParameterType.IsClass ? "?" : "")}.ToString());");
                }

                classBuilder.AppendLine("            request.Content = content;");
            }
            ...
            {
                classBuilder.AppendLine($"            {entry.Value.Name} = GetHeaderValue<{ToCompilableName(entry.Value.ParameterType)}>(response, \"{entry.Key}\");");
            }
