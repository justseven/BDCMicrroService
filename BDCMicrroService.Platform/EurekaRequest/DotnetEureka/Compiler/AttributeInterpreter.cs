...
namespace DotnetEureka.Compiler
{
    /// <summary>
    ...
    internal static class AttributeInterpreter
    {
        ...
        internal static bool CreateContentObjectIfSpecified(BodyAttribute bodyAttr, ParameterInfo bodyParam, StringBuilder clasStringBuilder)
        {
            ...
            {
                case Format.Json:
                    clasStringBuilder.AppendLine($"            var content = ConvertToJson({bodyParam.Name});");
                    break;
                case Format.Raw:
                    clasStringBuilder.AppendLine($"            var content = StreamRawContent({bodyParam.Name});");
                    break;
                default:
                    return false;
            }
            ...
            switch (format)
            {
                case Format.Json:
                    classBuilder.AppendLine(isAsyncCall
                        ? $"            return await ConvertToObjectAsync<{WebServiceClassGenerator.ToCompilableName(returnType)}>(response);"
                        : $"            return ConvertToObject<{WebServiceClassGenerator.ToCompilableName(returnType)}>(response);");
                    break;
                case Format.Raw:
                    classBuilder.AppendLine(isAsyncCall
        ...
        internal static void InterpretRequestParameter(ParameterInfo parameter, StringBuilder classBuilder)
        {
            ...
            classBuilder.AppendLine($"            reqParams.Add(\"{paramName}\", {parameter.Name}{(parameter.ParameterType.IsClass ? "?" : "")}.ToString());");
        }
        ...
        {
            classBuilder.AppendLine($"            return GetHeaderValue<{WebServiceClassGenerator.ToCompilableName(returnType)}>(response, \"{responseHeaderAttribute.Header}\");");
        }
    }
}
